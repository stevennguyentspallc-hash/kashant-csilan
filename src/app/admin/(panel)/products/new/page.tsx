...
export default function NewProductPage() {
  ...
  const [variants, setVariants] = useState<Variant[]>([
    { color_name: "", color_hex: "#ffffff", image_file: null, image_preview: null },
  ]);
    ...
    try {
      ...
      for (const [idx, variant] of variants.entries()) {
        ...
        if (variant.image_file) {
          ...
          const fileName = `${product.id}-${idx}-${Date.now()}.${ext}`;
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="font-serif text-3xl font-bold text-charcoal-900">Add Product</h1>
          <p className="text-charcoal-800/50 text-sm mt-1">Create a new product listing</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input name="name" required value={form.name} onChange={handleChange}
                ...
                placeholder="Luxe Pro Pedicure Chair" />
            </div>
            <div>
              <label className="text-xs uppercase tracking-wider text-charcoal-800/50 mb-1 block">Slug (auto-generated)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input name="dimensions" value={form.dimensions} onChange={handleChange}
                className="w-full border border-cream-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-gold-400 transition-colors"
                placeholder="28W x 52L x 45H in" />
            </div>
            <div>
          ...
          <div className="flex gap-6 mt-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" name="is_featured" checked={form.is_featured}
                onChange={handleChange} className="w-4 h-4 accent-gold-400" />

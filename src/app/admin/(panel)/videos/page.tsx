...
const EMPTY = { title: "", youtube_url: "", description: "" };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl md:text-3xl font-bold text-white">Videos</h1>
          <p className="text-white/50 text-sm mt-1">{videos.length} videos</p>
        </div>
              ...
              <div key={v.id}
                ...
                <div className="flex-1 p-4 flex items-center justify-between gap-3 min-w-0">
                  <div className="min-w-0">
                    ...
                    {v.description && (
                      <p className="text-xs text-charcoal-800/60 truncate mt-0.5">{v.description}</p>
                    )}
                    ...
                    <div className="mt-1.5">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full ${v.is_active ? "bg-green-500/15 text-green-400" : "bg-wood-900 text-charcoal-800/50"}`}>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <div className="flex flex-col gap-0.5 mr-1">
                      <button onClick={() => moveOrder(v.id, "up")} disabled={idx === 0}
                        className="text-charcoal-800/50 hover:text-white disabled:opacity-20 text-xs p-1">▲</button>
                      <button onClick={() => moveOrder(v.id, "down")} disabled={idx === videos.length - 1}
                        className="text-charcoal-800/50 hover:text-white disabled:opacity-20 text-xs p-1">▼</button>
                    </div>
                    <button onClick={() => toggleActive(v.id, v.is_active)}
                      className={`text-xs px-2.5 py-1.5 rounded-full border transition-colors ${v.is_active ? "border-wood-300 text-charcoal-800/60 hover:border-yellow-400" : "border-green-500/40 text-green-400"}`}>
                      {v.is_active ? "Hide" : "Show"}
                    </button>
                    <button onClick={() => openEdit(v)}
                      className="p-2 hover:bg-wood-100 rounded-lg text-charcoal-800/60 hover:text-white">
                      <Pencil size={14}/>
                    </button>
                    <button onClick={() => handleDelete(v.id)}
                      className="p-2 hover:bg-red-500/10 rounded-lg text-charcoal-800/60 hover:text-red-400">
                      <Trash2 size={14}/>
                    </button>
                  </div>
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-wood-50 border border-wood-800 w-full max-w-lg rounded-2xl shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))}
                  className={inputCls}
                  placeholder="Slian Solutions - Nail Salon Furniture"/>
              </div>
              <div>
                <label className="text-xs uppercase tracking-wider text-charcoal-800/60 mb-1 block">YouTube URL *</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={() => setShowForm(false)}
                  className="px-6 py-3.5 border border-wood-300 text-charcoal-800 text-sm uppercase rounded-full hover:border-white">
                  Cancel
                </button>
              </div>
            </div>

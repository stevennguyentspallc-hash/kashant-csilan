...
export default function ProductDetailPage({
  ...
  const currentImage = selectedVariant?.image_url ?? product.product_variants?.[0]?.image_url ?? null;
  ...
  return (
    <>
                  ...
                  {product.product_variants.map((v) => (
                    <button key={v.id} onClick={() => setSelectedVariant(v)}
                      className={`relative w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${
                        selectedVariant?.id === v.id ? "border-gold-400" : "border-cream-200 hover:border-charcoal-800/30"
                      }`}>
                      {v.image_url ? (
                        <Image src={v.image_url} alt={v.color_name} fill className="object-cover" />
                      ) : (
                        <div className="w-full h-full" style={{ backgroundColor: v.color_hex ?? "#ccc" }} />
                      )}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex flex-col">
              ...
              {product.product_variants && product.product_variants.length > 0 && (
                <div className="mb-8">
                  ...
                  <div className="flex gap-3">
                    {product.product_variants.map((v) => (
                      <button key={v.id} title={v.color_name} onClick={() => setSelectedVariant(v)}
              ...
              <div className="grid grid-cols-2 gap-4 mb-8 p-6 bg-white rounded-2xl border border-cream-100">
                <p className="col-span-2 text-xs uppercase tracking-widest text-charcoal-800/40 mb-2">Specifications</p>
                {product.dimensions && (
      ...
      {showModal && (
        <QuoteModal product={product} selectedVariant={selectedVariant} onClose={() => setShowModal(false)} />
      )}
    </>
  );
}
